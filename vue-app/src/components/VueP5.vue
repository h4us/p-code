...
export default {
  ...
  computed: {
    didLoad: {
      ...
      async set(v) {
        await this.$store.dispatch('isPlayingAction', v);
      }
    },
    ...
    doLoop: {
      ...
      set(v) {
        this.$store.dispatch('doLoopAction', v);
      }
    },
    ...mapState([
      'drawer',
    ...
    const p5fn = (sketch) => {
      ...
      sketch.setup = () => {
        ...
        this.capture = sketch.createCapture({
          ...
          this.filtered = sketch.createGraphics(this.capture.elt.videoWidth, this.capture.elt.videoHeight);
        });
        this.capture.hide();
      ...
      sketch.draw = () => {
        ...
        if (this.capture && this.capture.elt) {
          ...
          if (this.filtered) {
            ...
            this.filtered.filter(sketch.THRESHOLD, this.threashold);
          }
    ...
    this.$store.subscribeAction((action) => {
      ...
      if (action.type == 'camFaceAction') {
        ...
        this.capture = this.p5.createCapture({
          ...
          this.filtered = this.p5.createGraphics(this.capture.elt.videoWidth, this.capture.elt.videoHeight);
          this.takeSnapshot();
        });
        this.capture.hide();
      }
  ...
  methods: {
    takeSnapshot() {
      if (this.capture && this.capture.elt && this.filtered) {
        this.filtered.image(
          this.capture,
          0, 0,
          this.capture.elt.videoWidth, this.capture.elt.videoHeight
        );
        this.filtered.filter(this.p5.THRESHOLD, this.threashold);
        this.filteredSnapshotAction(this.filtered.elt.toDataURL());
      }
    },
    ...
    recieveFromExternal(_code) {
      let code = this.unpack(_code);
    ...
    getFilteredImage() {
      return this.filtered ? this.filtered.elt.toDataURL() : '';
    },

    requestFilteredImage() {
      if (this.filtered) {
        this.filteredSnapshotAction(this.filtered.elt.toDataURL());
      }
    },
